...
/// A supplement in the user's list with its usual dosage and time of day.
class _SupplementPlan {
  ...
  _SupplementPlan({required this.name, this.dosage = '', this.am = false, this.pm = false});

  Map<String, dynamic> toJson() => {'name': name, 'dosage': dosage, 'am': am, 'pm': pm};
}

/// Supplements tab: the user keeps a list of supplements and, for the date
/// selected in the shared calendar, marks which were taken (and when).
///
/// Storage (via [BackendServiceProvider]):
/// - `supplements` entry per day:
///   `{date, supplements: [{name, dosage, time, taken}]}` (read by the AI
///   assistant and insights).
/// - `tracked_lists/supplements`: the list with dosages, shared across days.
...
class _SupplementsScreenMainState extends State<SupplementsScreenMain> {
  static const _entryType = 'supplements';
  static const _listsType = 'tracked_lists';
  static const _listId = 'supplements';
  ...
  // The user's supplement list (shared across days).
  final List<_SupplementPlan> _mySupplements = [];
  ...
  final List<SupplementItem> _commonSupplements = const [
    SupplementItem(name: 'Vitamin D', description: 'Fat-soluble vitamin (D3 or D2)', icon: Icons.wb_sunny_outlined),
    SupplementItem(name: 'Probiotics', description: 'Live bacterial cultures', icon: Icons.bubble_chart_outlined),
    SupplementItem(name: 'Omega-3', description: 'Fish or algae oil (EPA/DHA)', icon: Icons.water_outlined),
    SupplementItem(name: 'Iron', description: 'Mineral, often prescribed for low iron', icon: Icons.fitness_center_outlined),
    SupplementItem(name: 'B12', description: 'Vitamin B12 (cobalamin)', icon: Icons.bolt_outlined),
    SupplementItem(name: 'Calcium', description: 'Mineral for bones and teeth', icon: Icons.shield_outlined),
    SupplementItem(name: 'Zinc', description: 'Trace mineral', icon: Icons.security_outlined),
    SupplementItem(name: 'Magnesium', description: 'Essential mineral', icon: Icons.auto_awesome_outlined),
    SupplementItem(name: 'Folic Acid', description: 'Vitamin B9 (folate)', icon: Icons.favorite_outlined),
    SupplementItem(name: 'Multivitamin', description: 'Combined vitamins and minerals', icon: Icons.medication_outlined),
  ];

  /// Supplements shown for the selected day: the list plus anything logged
  /// that day which has since been removed from the list.
  List<String> get _displayedSupplements {
    final names = _mySupplements.map((s) => s.name).toList();
    return [...names, ..._loggedNames.where((name) => !names.contains(name))];
  }

  _SupplementPlan? _planFor(String name) {
    for (final plan in _mySupplements) {
  ...
  Future<void> _load() async {
    ...
    if (userId == null) {
      setState(() {
        _loading = false;
        _loadError = 'Sign in to track your supplements.';
      });
      return;
    }
    ...
    try {
      ...
      debugPrint('SupplementsScreen: failed to load $dateKey: $error');
      if (!mounted || generation != _loadGeneration) return;
      setState(() {
        _loading = false;
        _loadError = "Couldn't load your supplements. Check your connection and try again.";
      });
    }
  }

  void _applyList(Map<String, dynamic>? data) {
    _mySupplements.clear();
    ...
    if (items is List) {
      ...
      for (final item in items) {
        ...
        _mySupplements.add(_SupplementPlan(
          name: name,
      ...
      // Nothing saved yet: start from the supplements entered during onboarding.
      for (final supplement in _appState.userSupplements) {
        final name = supplement.name.trim();
        if (name.isEmpty || _planFor(name) != null) continue;
        _mySupplements.add(_SupplementPlan(name: name, am: supplement.takesAM, pm: supplement.takesPM));
      }
    }
    _listLoaded = true;
  }
  ...
  void _applyDay(Map<String, dynamic>? data) {
    ...
    final logged = data?['supplements'];
    if (logged is! List) return;
    for (final supplement in logged) {
      if (supplement is! Map) continue;
      final name = '${supplement['name'] ?? ''}'.trim();
      if (name.isEmpty || _loggedNames.contains(name)) continue;
      _loggedNames.add(name);
      if (supplement['taken'] == true) _taken.add(name);
      final times = '${supplement['time'] ?? ''}'.toUpperCase().split(RegExp(r'[,&/ ]+'));
      _takenAM[name] = times.contains('AM');
      _takenPM[name] = times.contains('PM');
      final dosage = supplement['dosage'];
  ...
  void _saveDay() {
    ...
    final data = <String, dynamic>{
      'date': dateKey,
      'supplements': [
        for (final name in _displayedSupplements)
          {
            'name': name,
  ...
  void _saveList() {
    ...
    final data = <String, dynamic>{
      'entry_id': _listId,
      'items': [for (final plan in _mySupplements) plan.toJson()],
    };
  ...
  Future<void> _persist(String userId, String type, Map<String, dynamic> data) async {
    try {
      ...
      debugPrint('SupplementsScreen: failed to save $type: $error');
      if (mounted) {
        ScaffoldMessenger.maybeOf(context)?.showSnackBar(
          const SnackBar(content: Text("Couldn't save your supplements. Please try again.")),
        );
      }
    }
  }
  ...
  void _markAllTaken() {
    if (_loading) return;
    setState(() {
      _taken.addAll(_displayedSupplements);
      _dayLogged = true;
    });
    _saveDay();
  }
  ...
  void _addToMyList(String name, {String dosage = ''}) {
    setState(() {
      _mySupplements.add(_SupplementPlan(name: name, dosage: dosage));
      _inputError = null;
    });
    _saveList();
  ...
  void _addCustomSupplement() {
    ...
    if (text.isEmpty) {
      setState(() => _inputError = 'Enter a supplement name to add it.');
      return;
    }
    final lower = text.toLowerCase();
    final existing = _mySupplements.where((s) => s.name.toLowerCase() == lower);
    if (existing.isNotEmpty) {
      setState(() => _inputError = '${existing.first.name} is already in your list.');
      return;
    }
    // Typing one of the suggestions adds that suggestion.
    final common = _commonSupplements.where((s) => s.name.toLowerCase() == lower);
  ...
  void _removeSupplement(String name) {
    if (_loading) return;
    setState(() {
      _mySupplements.removeWhere((s) => s.name == name);
  ...
  String get _daySummary {
    ...
    final total = _displayedSupplements.length;
    if (total == 0) return 'Logged for $_dayPhrase: no supplements';
    return 'Logged for $_dayPhrase: ${_taken.length} of $total taken';
  }

  SupplementItem? _commonFor(String name) {
    for (final supplement in _commonSupplements) {
              ...
              child: _loading
                  ? const Center(
                      child: CircularProgressIndicator(
                        color: Colors.white,
                        semanticsLabel: 'Loading supplements',
                      ),
                    )
                  : _loadError != null
                      ? _buildLoadError()
  ...
  Widget _buildContent() {
    final displayed = _displayedSupplements;
    final myNames = _mySupplements.map((s) => s.name).toSet();
    final suggestions = _commonSupplements.where((s) => !myNames.contains(s.name)).toList();
            ...
            child: Column(
              ...
              children: [
                if (displayed.isNotEmpty) ...[
                  ...
                  ...displayed.map(_buildSupplementCard),
                ] else
                  _buildEmptyList(),
                ...
                if (suggestions.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 12),
                  ...suggestions.map(_buildAddSupplementCard),
                ],
  ...
  Widget _buildSupplementCard(String name) {
    final isTaken = _taken.contains(name);
    final plan = _planFor(name);
    final common = _commonFor(name);
    final dosage = _dosageFor(name);
    final description = common?.description ?? 'Custom supplement';
            ...
            children: [
              Row(
                children: [
                  ...
                  IconButton(
                    tooltip: 'Remove $name',
                    onPressed: () => _removeSupplement(name),
  ...
  Widget _buildAddSupplementCard(SupplementItem supp) {
    void add() => _addToMyList(supp.name);
    return Padding(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(supp.name, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white.withValues(alpha: 0.85))),
                        Text(supp.description, style: TextStyle(fontSize: 11, color: Colors.white.withValues(alpha: 0.6))),
                      ],
                    ),
                  ),
                  Container(
